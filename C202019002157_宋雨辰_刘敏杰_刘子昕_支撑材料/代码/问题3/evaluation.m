...
score=nan(302,5);
normalized_score=nan(302,5);
final_score=nan(302,1);
%find credit rating(C5)
[num,str,raw]=xlsread('class.xlsx',1,'','basic');
credit=char(raw(:,5));
temp=credit;
...
a=min(final_score);
b=max(final_score);
xlswrite('final_score_q3.xlsx',[score(:,1),normalized_score(:,1),score(:,2),normalized_score(:,2),score(:,3),normalized_score(:,3),score(:,4),normalized_score(:,4),score(:,5),final_score]);
